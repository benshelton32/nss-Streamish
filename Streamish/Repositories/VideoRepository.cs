    ...
    public class VideoRepository : BaseRepository, IVideoRepository
    {
        ...
        public Video GetByIdWithComments(int id)
        {
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
                          SELECT v.Id, v.Title, v.Description, v.Url, v.DateCreated AS VideoDateCreated, v.UserProfileId AS VideoCreatorId, 
		                         vup.Name AS VideoCreatorName, vup.ImageUrl AS VideoCreatorAvatar, 
		                         vup.DateCreated AS VideoCreatorCreatedDate, vup.Email AS VideoCreatorEmail,
		                         c.Id AS CommentId, c.Message, c.UserProfileId AS CommentUserProfileId, cup.Name AS CommentorName,
		                         cup.ImageUrl AS CommentorAvatar, cup.DateCreated AS CommentorDateCreated, cup.Email AS CommentorEmail
                            FROM Video v
                       LEFT JOIN Comment c ON c.VideoId = v.Id
                            JOIN UserProfile vup ON vup.Id = v.UserProfileId
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            video = new Video()
                            {
                                ...
                                DateCreated = DbUtils.GetDateTime(reader, "VideoDateCreated"),
                                Url = DbUtils.GetString(reader, "Url"),
                                UserProfileId = DbUtils.GetInt(reader, "VideoCreatorId"),
                                UserProfile = new UserProfile()
                                {
                                    Id = DbUtils.GetInt(reader, "VideoCreatorId"),
                                    Name = DbUtils.GetString(reader, "VideoCreatorName"),
                                    Email = DbUtils.GetString(reader, "VideoCreatorEmail"),
                                    DateCreated = DbUtils.GetDateTime(reader, "VideoCreatorCreatedDate"),
                                },
                                Comments = new List<Comment>()
                            };

                            if (DbUtils.IsNotDbNull(reader, "VideoCreatorAvatar"))
                            {
                                video.UserProfile.ImageUrl = DbUtils.GetString(reader, "VideoCreatorAvatar");
                            }
                                ...
                                if (DbUtils.IsNotDbNull(reader, "CommentId"))
                                {
                                    video.Comments.Add(new Comment()
                                    {
                                        ...
                                        UserProfile = new UserProfile()
                                        {
                                            ...
                                            Name = DbUtils.GetString(reader, "CommentorName"),
                                            Email = DbUtils.GetString(reader, "CommentorEmail"),
                                            DateCreated = DbUtils.GetDateTime(reader, "CommentorDateCreated"),
                                        },
                                    });

                                    if (DbUtils.IsNotDbNull(reader, "CommentorAvatar"))
                                    {
                                        video.UserProfile.ImageUrl = DbUtils.GetString(reader, "CommentorAvatar");
                                    }
                                }
